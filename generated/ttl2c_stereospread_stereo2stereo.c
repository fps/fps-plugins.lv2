...
static LV2_Handle plugin_instantiate_desc(const LV2_Descriptor *descriptor, double sample_rate, const char *bundle_path, const LV2_Feature *const *features) {
    ...
    lv2_log_note(&instance->logger, "Instantiating a http://fps.io/plugins/stereospread-stereo2stereo\n");
...
static void plugin_cleanup_desc(LV2_Handle instance) {
    ...
    lv2_log_note(&tinstance->logger, "Cleaning up a http://fps.io/plugins/stereospread-stereo2stereo\n");
...
static void plugin_run_desc(LV2_Handle instance, uint32_t sample_count) {
    if (plugin_callbacks.run) {
        ...
        const plugin_port_inl_t inl = { .data = ((float*)((plugin_t*)instance)->ports[0]) };
        const plugin_port_inr_t inr = { .data = ((float*)((plugin_t*)instance)->ports[1]) };
        const plugin_port_outl_t outl = { .data = ((float*)((plugin_t*)instance)->ports[2]) };
        const plugin_port_outr_t outr = { .data = ((float*)((plugin_t*)instance)->ports[3]) };
        const plugin_port_drywet_t drywet = { .data = ((float*)((plugin_t*)instance)->ports[4])[0] };
        const plugin_port_length_t length = { .data = ((float*)((plugin_t*)instance)->ports[5])[0] };

        plugin_callbacks.run(tinstance, sample_count, inl, inr, outl, outr, drywet, length);
    }
}
