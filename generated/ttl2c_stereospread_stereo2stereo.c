...
static void plugin_run_desc (LV2_Handle instance, uint32_t sample_count) 
{
    if (plugin_callbacks.run) 
    {
        ...
        plugin_port_inl_t const inl = { .data = ((float*)((plugin_t*)instance)->ports[0]) };
        plugin_port_inr_t const inr = { .data = ((float*)((plugin_t*)instance)->ports[1]) };
        plugin_port_outl_t const outl = { .data = ((float*)((plugin_t*)instance)->ports[2]) };
        plugin_port_outr_t const outr = { .data = ((float*)((plugin_t*)instance)->ports[3]) };
        plugin_port_drywet_t const drywet = { .data = ((float*)((plugin_t*)instance)->ports[4])[0] };
        plugin_port_length_t const length = { .data = ((float*)((plugin_t*)instance)->ports[5])[0] };
...
{
    "http://dfdx.eu/plugins/stereospread-stereo2stereo",
